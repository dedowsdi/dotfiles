...
augroup twig_filetype
  au!
  exe "autocmd BufNewFile,BufRead" join(s:gl_exts, ",") "set filetype=glsl"
augroup end
...
set fileencodings=ucs-bom,utf-8,gbk,cp936,big5,gb18030,latin1
set textwidth=80
set tabstop=4
...
" following semantic triggers will break ultisnips suggestion
 let g:ycm_semantic_triggers = {'c':['re!\w{5}'], 'cpp':['re!\w{5}']}
...
let s:maps = [
   ...
   \ ['<c-e>ds',     'n',  0, ['c'],        '<Plug>CdefCreateSourceFile'],
   ...
   \ ['<c-e>p',      'n',  1, ['c'],        'viw<esc>bi(*<esc>lviw<esc>a)->'],
   \ ['<c-e>p',      'i',  0, ['c'],        '<esc><c-e>p'],
   \ ['<c-e>,',      'nv',  0, [],           ':EasyAlign * /,/ {"stl":0, "l":0}<CR>'],
   \ ['<c-e>l',      'nv',  0, [],           ':EasyAlign * /[+\-*\/%]/ {"stl":0, "l":1, "r":1}<CR>'],
   \
   \ ['<c-j>',       'i',  1, ['c'],        '->'],
   \
